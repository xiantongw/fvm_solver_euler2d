...
//   // calculate smax
//   double vL, vR, sLmax, sRmax, smax;
//   vL = (uL[1] / uL[0]) * n[0] + (uL[2] / uL[0]) * n[1];
//   vR = (uR[1] / uR[0]) * n[0] + (uR[2] / uR[0]) * n[1];
//   sLmax = max(0.0, vL+cL);
//   sRmax = max(0.0, vR+cR);
//   smax = max(sLmax, sRmax);

//   F_hat[0] = 0.5 * (FL_hat[0] + FR_hat[0]) - 0.5 * smax * (uR[0] - uL[0]);
//   F_hat[1] = 0.5 * (FL_hat[1] + FR_hat[1]) - 0.5 * smax * (uR[1] - uL[1]);
//   F_hat[2] = 0.5 * (FL_hat[2] + FR_hat[2]) - 0.5 * smax * (uR[2] - uL[2]);
//   F_hat[3] = 0.5 * (FL_hat[3] + FR_hat[3]) - 0.5 * smax * (uR[3] - uL[3]);
