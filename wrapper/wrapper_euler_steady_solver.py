...
def c_euler_solver_main(mesh, param):
    ...
    c_Centroid = ((c_double * 2) * np.shape(mesh.E)[0])()
    ...
    for i in range(np.shape(mesh.E)[0]):
        for j in range(2):
            c_Centroid[i][j] = mesh.Centroid[i, j]
    
    norder = param["norder"]

    solver.euler_solver_main(byref(cfd_param), byref(mesh_param), byref(boudary_param), c_int(norder),
                            c_E, c_V, c_I2E, c_B2E, c_In, c_Bn, c_Area, c_Centroid,
                            c_state_vectors,
                            c_int(param["MAXITER"]), c_double(param["eps"]))
    
    state_vectors = np.zeros([np.shape(mesh.E)[0], 4])


    for i in range(np.shape(mesh.E)[0]):
        for j in range(4):
            state_vectors[i, j] = c_state_vectors[i][j]
